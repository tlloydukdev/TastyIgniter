...
if ($locationCurrent->hasDelivery() OR $locationCurrent->hasCollection()) { ?>
    ...
    >
        ...
        <?php if ($locationCurrent->hasCollection()) { ?>
            <label style="background-color: #632FEC; border-color: #632FEC;" class="btn btn-<?= $location->orderTypeIsCollection() ? 'primary' : 'light'; ?> <?= $location->orderTypeIsCollection() ? 'active' : ''; ?>">
                <input
                    type="radio"
                    name="order_type"
